...
一次跑完三件事:
  1. 建表/加列/菜单种子幂等重放(payment_record/payment_alloc CREATE IF NOT EXISTS、
     supplier.settle_days information_schema 判存、sys_menu 40/4001~4003 INSERT IGNORE)
...
聚合语句从 erp-finance/src/main/resources/mapper/PaymentQueryMapper.xml 原文提取
...
def main():
    ...
    try:
        ...
        # ── 哑元:供应商 + 三张采购单(草稿不计应付)+ 两条 NORMAL 付款(分摊 60/40)、一条 VOIDED(分摊 50)──
        cur.execute(f"INSERT INTO supplier (id, name, status) VALUES ({SUPPLIER_ID},'__val__资金供应商',1)")
        cur.execute(
            f"INSERT INTO purchase_order (id, po_no, supplier_id, warehouse_id, status, total_amount) VALUES "
            f"({PO_A},'__val__POA',{SUPPLIER_ID},990031,'AUDITED',100),"
            f"({PO_B},'__val__POB',{SUPPLIER_ID},990031,'DRAFT',30),"
            f"({PO_C},'__val__POC',{SUPPLIER_ID},990031,'AUDITED',50)")
