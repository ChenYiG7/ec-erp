...
public class AlertEngine {
    ...
    private AlertEvent lowStockRule() {
        List<InventoryQueryApi.InventoryView> hits = scanPages(pageNo -> inventoryQueryApi.pageInventory(
                ...
                row -> row.qtyAvailable() != null && row.qtyAvailable() <= props.getLowStockThreshold());
        if (hits.isEmpty()) {
            return null;
        }
        List<String> details = hits.stream().limit(props.getTopN())
        ...
        return AlertEvent.builder()
                ...
                .content(detailContent(StrUtil.format("低库存 SKU 共 {} 条(阈值≤{})",
                        hits.size(), props.getLowStockThreshold()), details, hits.size()))
                .build();
    }

    /** 发货超时:分页扫 WAIT_SHIP 订单,下单时间早于 now-N 小时命中 */
    private AlertEvent shipTimeoutRule(LocalDateTime now) {
        LocalDateTime deadline = now.minusHours(props.getShipTimeoutHours());
        ...
        return AlertEvent.builder()
                ...
                .content(detailContent(StrUtil.format("待发货超 {} 小时订单共 {} 单",
                        props.getShipTimeoutHours(), hits.size()), details, hits.size()))
                .build();
    }

    /** 退款异常:分页扫 REFUNDED 售后单,窗口内按店铺聚合计数,达阈值店铺命中 */
    private AlertEvent refundRule(LocalDateTime now) {
        LocalDateTime since = now.minusHours(props.getRefundWindowHours());
        ...
        List<Map.Entry<Long, Integer>> hitShops = countByShop.entrySet().stream()
                .filter(entry -> entry.getValue() >= props.getRefundCountThreshold())
                .sorted((a, b) -> b.getValue() - a.getValue())
                .toList();
        if (hitShops.isEmpty()) {
            return null;
        }
        List<String> details = hitShops.stream().limit(props.getTopN())
        ...
        return AlertEvent.builder()
                ...
                .content(detailContent(StrUtil.format("近 {} 小时退款≥{}单的店铺共 {} 家",
                        props.getRefundWindowHours(), props.getRefundCountThreshold(), hitShops.size()),
    ...
    private AlertEvent slowMovingRule() {
        ...
        Map<Long, Integer> soldBySku = soldBySku(stocked);
        ...
        return AlertEvent.builder()
                ...
                .content(detailContent(StrUtil.format("近 {} 天零动销且有库存 SKU 共 {} 条",
                        props.getSlowMovingDays(), hits.size()), details, hits.size()))
                .build();
    }
    ...
    private AlertEvent overstockRule() {
        ...
        for (InventoryQueryApi.InventoryView row : stocked) {
            ...
            double dailyAvg = (double) sold / props.getSlowMovingDays();
            long coverDays = (long) Math.ceil(row.qtyAvailable() / dailyAvg);
            if (coverDays < props.getOverstockDays()) {
                continue;
            }
            total++;
            if (hits.size() < props.getTopN()) {
        ...
        return AlertEvent.builder()
                ...
                .content(StrUtil.format("库存可支撑 ≥{} 天的 SKU 共 {} 条,明细: {}{}",
                        props.getOverstockDays(), total, String.join("; ", hits), suffix))
                .build();
    }
    ...
    /** 批量取动销窗口内销量合计(空集不触契约) */
    private Map<Long, Integer> soldBySku(List<InventoryQueryApi.InventoryView> rows) {
        return salesQueryApi.sumQtyBySku(rows.stream()
                .map(InventoryQueryApi.InventoryView::skuId)
                .collect(Collectors.toSet()), props.getSlowMovingDays());
    }
