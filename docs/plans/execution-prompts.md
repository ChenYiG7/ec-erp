...
### 12) #24 adapter Playbook(仓库根;资质解卡后)

```text
执行 docs/plans/24-adapter-playbook.md(#24 平台 adapter 批量扩展 SOP):本次目标 = <二选一:产出/修订 Playbook 手册 / 以 <平台名> 按手册演练接入>。
...
```text
执行 docs/plans/p3-ai-workflow-scheduling.md。
...
### P3-3) 聊天页体验(erp-web/ 开会话)

```text
执行 docs/plans/p3-chat-ux.md(触发条件:阅读痛点真实出现且引库已拍板——本次视为已拍板)。
