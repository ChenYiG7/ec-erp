...
<template>
  <div class="table-box">
    <ProTable
      ref="proTableRef"
      page-id="/purchase/inbounds"
      title="入库单"
      :columns="columns"
      :request-api="purchaseInboundApi.page"
    >
      <!-- 工具栏左:新建入库单(#10 人工扩展槽;按钮权限收口在页面侧 v-auth;toolbarLeft prop 的 auth 属性无效,禁用) -->
      <template #toolbarLeft>
        <el-button v-auth="'purchase:inbound:add'" type="primary" :icon="CirclePlus" @click="openCreateForm"
          >新建入库单</el-button
        >
      </template>
      ...
      <template #operation="scope">
        <el-button
          ...
          v-auth="'purchase:inbound:confirm'"
          type="primary"
          link
        ...
        <el-button
          v-if="scope.row.status === 'PENDING'"
          v-auth="'purchase:inbound:cancel'"
          type="danger"
          link
...
const columns: ColumnProps<PurchaseInboundResponse>[] = [
  ...
  { prop: 'warehouseId', label: '仓库', width: 130, enum: fetchWarehouseOptions },
  {
    ...
    tag: true,
    enum: [
      { label: '待入库', value: 'PENDING', tagType: 'warning' },
...
const onConfirm = async (row: PurchaseInboundResponse) => {
  await ElMessageBox.confirm(`确认入库单 ${row.inboundNo} 收货入库吗?确认后将按明细写入库存,不可回退。`, '提示', {
    type: 'warning',
  })
