  ...
  <el-dialog v-model="visible" :title="title" width="560px" :close-on-click-modal="false" destroy-on-close>
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="110px">
      ...
      <el-form-item label="字典标签" prop="dictLabel">
        <el-input v-model="formData.dictLabel" placeholder="请输入字典标签" clearable />
      </el-form-item>
      <el-form-item label="字典值" prop="dictValue">
        <el-input v-model="formData.dictValue" placeholder="请输入字典值" clearable />
      </el-form-item>
...
const formData = ref<SysDictSaveRequest>({} as SysDictSaveRequest)
...
const open = (m: 'add' | 'edit', row?: SysDictResponse) => {
  mode.value = m
  editId.value = row?.id
  title.value = (m === 'add' ? '新增' : '编辑') + '字典管理'
  formData.value = {} as SysDictSaveRequest
  if (row) {
...
const handleSubmit = async () => {
  ...
  try {
    if (mode.value === 'add') {
      await sysDictApi.create(formData.value)
    } else {
      await sysDictApi.update(editId.value!, formData.value)
    }
