...
<template>
  <div class="table-box">
    <ProTable ref="proTableRef" page-id="/fulfill/delivery-orders" title="发货单" :columns="columns" :request-api="deliveryOrderApi.page">
      <!-- 工具栏左:新建发货单(#11 人工扩展槽;按钮权限收口在页面侧 v-auth;toolbarLeft prop 的 auth 属性无效,禁用) -->
      <template #toolbarLeft>
        <el-button v-auth="'fulfill:delivery:add'" type="primary" :icon="CirclePlus" @click="openCreateForm">新建发货单</el-button>
      </template>
      ...
      <template #operation="scope">
        <el-button v-if="scope.row.status === 'PENDING'" v-auth="'fulfill:delivery:edit'" type="primary" link @click="openEditForm(scope.row)">编辑</el-button>
        <el-button v-if="scope.row.status === 'PENDING'" v-auth="'fulfill:delivery:ship'" type="primary" link @click="onShip(scope.row)">确认发货</el-button>
        <el-button v-if="scope.row.status === 'SHIPPED'" v-auth="'fulfill:delivery:deliver'" type="warning" link @click="onDeliver(scope.row)">标记签收</el-button>
        <el-button v-if="scope.row.status === 'PENDING'" v-auth="'fulfill:delivery:cancel'" type="danger" link @click="onCancel(scope.row)">取消</el-button>
      </template>
    </ProTable>
...
const columns: ColumnProps<DeliveryOrderResponse>[] = [
  ...
  { prop: 'type', label: '类型', width: 110, tag: true, enum: [{ label: '自发货', value: "SELF_FULFILL", tagType: 'primary' }, { label: 'FBA', value: "FBA", tagType: 'success' }, { label: '海外仓', value: "OVERSEAS", tagType: 'warning' }] },
  { prop: 'status', label: '状态', width: 100, tag: true, enum: [{ label: '待发货', value: "PENDING", tagType: 'warning' }, { label: '已发货', value: "SHIPPED", tagType: 'primary' }, { label: '已签收', value: "DELIVERED", tagType: 'success' }, { label: '已取消', value: "CANCELLED", tagType: 'info' }] },
...
const onShip = async (row: DeliveryOrderResponse) => {
  await ElMessageBox.confirm(`确认发货单 ${row.deliveryNo} 出库发货吗?确认后将按明细扣减出库仓库存,不可回退。`, '提示', { type: 'warning' })
