...
export function parseSpec(specPath) {
  ...
  if (spec.pageType === 'chat') {
    ...
    if (!spec.typesFrom) {
      fail(
        'chat 模式缺 typesFrom=(契约类型复用源 <module>/<domain>,如 ai/chat;模板不自产会话类型)',
      );
    }
    ...
    if (spec.roles.length) {
      ...
      if (spec.emptyText || spec.placeholder) {
        warnings.push(
          'chat 角色模式忽略 emptyText=/placeholder=(文案走 role= 行的 empty=/placeholder=)',
        );
      }
...
function parseFieldLine(line, lineNo, fieldByName) {
      ...
      field.enumMap = v.split('|').map((pair) => {
        ...
        if (parts.length < 2 || !parts[0] || !parts[1]) {
          failAt(
            specPath,
            lineNo,
            `字段 ${name} 的 enum 格式非法: ${pair}(应为 值:标签[:tagType],如 1:启用:success)`,
          );
        }
  ...
  if (!field.label) {
    console.warn(
      `[gen:page] spec:${lineNo} 字段 ${name} 缺 label=中文标注(列/表单标题将回落字段名)`,
    );
  }
