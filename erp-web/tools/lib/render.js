 ...
 */
import { pageParameterEntries, refName, resolveRef } from './openapi.js';
...
export function buildContext({ spec, endpoints, doc, baseSeg, warnings = [] }) {
  const ctx = {
    ...
    nested: new Map(),
    skipNested: new Set([
      `${spec.entity}Response`,
      `${spec.entity}SaveRequest`,
      `${spec.entity}Query`,
    ]),
    warnings,
  };
  ...
  ctx.actionBodyTypes = ctx.actionEndpoints
    ...
    .map(({ act, body }) => ({
      tsName: `${spec.entity}${pascal(act.verb)}Request`,
      schema: body.schema,
    }));
...
export function renderTypes(ctx) {
  ...
  if (ctx.requestSchema?.schema?.properties) {
    ...
    for (const [key, sub] of Object.entries(ctx.requestSchema.schema.properties)) {
      ...
      out.push(
        `  ${key}${ctx.requestSchema.schema.required?.includes(key) ? '' : '?'}: ${meta.ts}`,
      );
    }
    out.push('}', '');
  }
  ...
  if (ctx.queryFields.length) {
    out.push(
      `/** ${ctx.nameZh}分页查询入参(不含分页参数) */`,
      `export interface ${ctx.queryName} {`,
    );
...
export function renderApi(ctx) {
  ...
  if (typeImports.length) {
    lines.push(
      `import type { ${typeImports.join(', ')} } from '@/api/interface/${ctx.module}/${ctx.domain}'`,
    );
  }
  ...
  if (ctx.endpoints.detail) {
    lines.push(
      `  /** 详情 */`,
      `  detail: (id: number) => http.get<${ctx.responseName}>(\`${ctx.basePath}/\${id}\`),`,
    );
  }
  ...
  if (!ctx.readonly && ctx.endpoints.remove) {
    lines.push(
      `  /** 删除 */`,
      `  remove: (id: number) => http.delete<boolean>(\`${ctx.basePath}/\${id}\`),`,
    );
  }
...
export function renderIndex(ctx) {
  ...
  if (hasOperation) {
    colLines.push(
      `  { prop: 'operation', label: '操作', fixed: 'right', width: ${operationWidth(ctx)} }`,
    );
  }
  ...
  if (ctx.responseSchema) {
    scr.push(
      `import type { ${ctx.responseName} } from '@/api/interface/${ctx.module}/${ctx.domain}'`,
    );
  }
  if (hasForm) {
  ...
  if (hasForm) {
    scr.push(
      `const openForm = (mode: 'add' | 'edit', row?: ${ctx.responseSchema ? ctx.responseName : 'any'}) => {`,
    );
    scr.push(`  formRef.value?.open(mode, row)`, `}`, '');
  }
  if (hasDelete) {
    scr.push(
      `const handleDelete = async (row: ${ctx.responseSchema ? ctx.responseName : 'any'}) => {`,
    );
    scr.push(
...
function elImportsFor(formFields) {
  ...
  const usesInputNumber = formFields.some(
    (f) => !f.dict && !f.enumMap && f.ts === 'number' && !f.money,
  );
...
export function renderForm(ctx) {
  ...
  const formItem = (f) => {
    ...
    if (f.money) {
      lines.push(
        `        <!-- 金额按后端 DECIMAL(12,4) 字符串直存直显,禁 parseFloat/Number 参与计算 -->`,
      );
    }
  ...
  const scr = [
    ...
    ...(() => {
      const names = [
        ...(needSaveType ? [saveName] : []),
        ...(ctx.responseSchema ? [ctx.responseName] : []),
      ];
      return names.length
...
export function renderChatIndex(ctx) {
  ...
  if (hasRoles) {
    hintBlock = [
      ...
      ctx.roles
        .map(
          (r) => `  ${r.enum}: { empty: '${esc(r.empty)}', placeholder: '${esc(r.placeholder)}' }`,
        )
        .join(',\n'),
      '}',
      '',
  ...
  return [
    ...
    ...(hasRoles
      ? [`import type { ${roleType} } from '@/api/interface/${ctx.module}/${ctx.domain}'`]
      : []),
