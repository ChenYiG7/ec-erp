...
export function verifyChatEndpoints(doc, chatBase) {
  ...
  const missing = required.filter(
    ([method, suffix]) => !doc.paths?.[chatBase + suffix]?.[method.toLowerCase()],
  );
  if (missing.length) {
    fail(
...
export function pageParameterEntries(op, doc) {
  ...
  const flatSearch = params.filter(
    (pr) => pr.in === 'query' && !pageKeys.includes(pr.name) && !pr.schema?.$ref,
  );
  const flat = params.filter((pr) => pr.in === 'query' && pageKeys.includes(pr.name));
  if (flat.length) {
    return {
      flat: true,
      fields: flatSearch.map((pr) => ({
        name: pr.name,
...
export function isPageSchema(schema) {
  return Boolean(
    schema?.properties && 'records' in schema.properties && 'total' in schema.properties,
  );
}
  ...
  return (
    ...
    (op.parameters || [])
      .filter(
        (pr) =>
          pr.in === 'query' &&
          !['pageNo', 'pageSize', 'pageNum'].includes(pr.name) &&
          !pr.schema?.$ref,
      )
      .map((pr) => ({
        name: pr.name,
        description: pr.description || '',
        schema: pr.schema || { type: 'string' },
      }))
  );
}
