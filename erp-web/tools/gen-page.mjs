...
if (spec.pageType === 'chat') {
  ...
  for (const f of ctx.fields) {
    if (!f.known && ctx.responseSchema?.properties) {
      ctx.warnings.push(
        `spec 字段 ${f.name} 不在后端响应 schema 中,列展示恒空;确认字段名或后端 Response 是否漏字段`,
      );
    }
  }
  ...
  if (formContent) {
    outputs.push([
      path.posix.join(viewDir, `components/${ctx.entity}Form.vue`).replaceAll('\\', '/'),
      formContent,
    ]);
  }
}
...
console.log(
  `[gen:page] ${ctx.nameZh}: module=${ctx.module} domain=${ctx.domain} pageType=${ctx.pageType || 'crud'}` +
    (ctx.pageType === 'chat'
      ? ` chatBase=${ctx.chatBase}`
      : ` base=${ctx.basePath} readonly=${ctx.readonly}`) +
