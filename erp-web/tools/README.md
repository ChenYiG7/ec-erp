...
- **守卫前置**:缺快照 / spec 缺头 / 未知键 / 未知 role / `todoId` 未登记 TODO.md,一律报错退出(带行号),禁静默降级。
- **冒烟自测**(不碰真快照与业务页面):

  ```bash
  GEN_OPENAPI_PATH=tools/test/smoke-openapi.json node tools/gen-page.mjs --spec tools/test/smoke-spec.txt
